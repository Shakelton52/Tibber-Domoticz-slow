...
#
"""
<plugin key="Tibber" name="Tibber API 1.231" author="flopp999" version="1.232s" wikilink="https://github.com/Shakelton52/Tibber-Domoticz-slow" externallink="https://tibber.com/se/invite/8af85f51">
    <description>
...
class BasePlugin:
    ...
    def _callback(pkg):
        ...
        Domoticz.Log('Realtime power={0}, powerProduction={1}'.format(realtimedata['power'],realtimedata['powerProduction']))
        tibber_connection.close_connection()
    ...
    def onHeartbeat(self):
        ...
        if self.RealTime is True and self.AllSettings is True and self.Count == 2 and PUlse == 1:
            ...
            async def LivePowerEvery():
                try:
                    async with Client(transport=WebsocketsTransport(url="wss://websocket-api.tibber.com/v1-beta/gql/subscriptions", init_payload={"token": self.AccessToken}, headers={"User-Agent": "Domoticz/1.232s"}, ping_interval=10 ) ) as session:
        ...
        if self.Count == 14 and MinuteNow < 59 and self.CurrentPriceUpdated is False and self.Subscription is True:
            if not _plugin.GetDataCurrent.Connected() and not _plugin.GetDataCurrent.Connecting():
                ...
                Domoticz.Log("CurrentPrice Updated Loop")
                _plugin.GetDataCurrent.Connect()
        if ( self.Count == 1 or self.Count == 5 or self.Count == 9 or self.Count == 14 or self.Count == 19 )  and MinuteNow == 59 and self.CurrentPriceUpdated is True:
            self.CurrentPriceUpdated = False
            self.Count = 13
        ...
        if self.Count >= 19: #### !!!!!!!!!! DO NOT CHANGE TIbber might lock your api-Key !!!!!!!!!!!!!!!
            self.Count = 0
